...
public class GetLancamentosByMesQueryHandler(ILancamentoRepository repository)
    : IRequestHandler<GetLancamentosByMesQuery, IEnumerable<LancamentoDto>>
{
    ...
    {
        var lancamentos = await repository.GetByMesAnoAsync(request.Mes, request.Ano, cancellationToken);
