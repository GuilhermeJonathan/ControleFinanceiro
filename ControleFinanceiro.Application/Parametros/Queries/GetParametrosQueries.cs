...
public record MoedaParamDto(int Id, string Codigo, string Nome, int Ordem, bool Ativo, bool IsSystem);
    ...
    : IRequestHandler<GetMoedasQuery, List<MoedaParamDto>>
{
    ...
    {
        var list = await repo.GetAllAsync(ct);
        return list.Select(x => new MoedaParamDto(x.Id, x.Codigo, x.Nome, x.Ordem, x.Ativo, x.IsSystem)).ToList();
    }
}
